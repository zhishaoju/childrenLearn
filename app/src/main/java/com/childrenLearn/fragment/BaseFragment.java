...
import com.childrenLearn.Net.NetInterface;
import com.childrenLearn.utils.Contants;
import com.childrenLearn.utils.JsonUtils;
import com.childrenLearn.utils.LogUtil;
import com.childrenLearn.utils.ToastUtil;
  ...
  protected void post(String url, String params) {
    ...
    if (JsonUtils.isJsonObject(params)) {
      ...
      try {
        ...
        Request request = new Request.Builder().post(requestBody).url(url).tag(mTag).build();
        mHttpClient
            .newCall(request)
            .enqueue(
                new Callback() {
                  ...
                  public void onFailure(Call call, IOException e) {
                    LogUtil.showELog(
                        TAG,
                        "post(String url, String params) "
                            + "onFailure(Call call, IOException e) e = "
                            + e.toString());
        ...
        try {
          if (JsonUtils.isJsonObject(resposeStr)) {
            ...
            try {
              ...
              String state = JsonUtils.jsonObjectStringGetValue(jsonObject, "state");
              String msgPost = JsonUtils.jsonObjectStringGetValue(jsonObject, "msg");
              ToastUtil.show(msgPost);
        ...
        try {
          if (JsonUtils.isJsonObject(sdata)) {
            ...
            try {
              ...
              String state = JsonUtils.jsonObjectStringGetValue(jsonObject, "state");
              String msgGet = JsonUtils.jsonObjectStringGetValue(jsonObject, "msg");
              ToastUtil.show(msgGet);
              if (Contants.NetStatus.OK.equals(state)) {
